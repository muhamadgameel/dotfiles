...
local function noremap_buf(bufnr, mode, keys, action, category, unique_identifier, description)
    if category == nil then
        vim.api.nvim_buf_set_keymap(bufnr, mode, keys, action, {
            noremap = true,
            silent = true,
        })
    else
        Mapper.map_buf(bufnr, mode, keys, action, {
            noremap = true,
            silent = true,
...
-- noremap buffer
for _, mode in pairs(modes) do
    M[mode .. "noremap_buf"] = function(bufnr, keys, action, category, unique_identifier, description)
...
-- map
for _, mode in pairs(modes) do
    M[mode .. "map"] = function(keys, action, category, unique_identifier, description) map(mode, keys, action, category, unique_identifier, description) end
end
